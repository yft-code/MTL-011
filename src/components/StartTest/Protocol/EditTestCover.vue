    ...
    methods: {
        ...
        getCpuData(){
            return new Promise(resolve => {
                this.$codePost("/protocolTest/get_cpu_ladder/", { operation: "获取CPU天梯数据", failed: true }).then(res => {
                    if (res.code == 200) {
        ...
        getCoverData() {
            return new Promise(resolve => {
                ...
                this.$codePost("/protocolTest/get_model_coverage/", list, { operation: "获取兼容性覆盖情况", failed: true }).then(res => {
                    if (res.code == 200) {
        ...
        handleClose(item, isCPU) {
            ...
            this.$selfConfirm(confirmText, '提示', { confirmButtonText: '确定', cancelButtonText: '取消' }).then(() => {
                if(!isCPU){
                    let list = { id: item.id };
                    this.$codePost("/protocolTest/delete_model_coverage/", list, { operation: "删除", success: true, failed: true }).then(res => {
                        if (res.code == 200) {
                            this.getCoverData();
                        }
                    })
                    ...
                    // 删除cpu覆盖项代码
                    let list = {name: item}
                    this.$codePost("/protocolTest/delete_cpu_coverage/", list, { operation: "删除cpu覆盖项", success: true, failed: true }).then(res => {
                        if (res.code == 200) {
                            this.handleReset();
                        }
                    })
        ...
        confirmAdd() {
            if(this.dialogMessage == "新增cpu等级"){
                ...
                if(!hasError){
                    ...
                    //填写无误，提交
                    this.$codePost("/protocolTest/add_cpu_coverage/", this.cpuForm, { operation: "新增cpu覆盖项", success: true, failed: true }).then(res => {
                        if (res.code == 200) {
                ...
                if (!hasError) {
                    //填写无误，提交
                    this.$codePost("/protocolTest/update_model_coverage/", this.coverForm, { operation: "新增覆盖项", success: true, failed: true }).then(res => {
                        if (res.code == 200) {
                            this.handleReset()
                        }
                    })
