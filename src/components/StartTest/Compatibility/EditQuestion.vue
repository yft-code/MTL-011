...
export default {
    ...
    methods: {
        //获取问题分类
        getQuestionClassify() {
            this.$codePost("/expertCompatibility/get_expert_test_question_classify/", {}, { operation: "获取选项", failed: true }).then(res => {
                if (res.code == 200) {
        ...
        confirmEditQuetion() {
            ...
            if (!hasError) {
                ...
                this.$codePost("/expertCompatibility/" + postApp + "/", list, { operation: this.editQuestionDialogTitle, success: true, failed: true }).then(res => {
                    if (res.code == 200) {
