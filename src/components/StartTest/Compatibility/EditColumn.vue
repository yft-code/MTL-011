...
export default {
    ...
    methods: {
        ...
        async getCustomCols() {
            ...
            await this.$codePost("/expertCompatibility/get_custom_cols/", list, { operation: "获取自定义列", failed: true }).then(res => {
                if (res.code == 200) {
        ...
        confirmEdit() {
            ...
            if (!hasError) {
                ...
                this.$codePost("/expertCompatibility/edit_cols/", list, { operation: "编辑列", success: true, failed: true }).then(res => {
                    if (res.code == 200) {
                        this.$emit("afterEdit");
