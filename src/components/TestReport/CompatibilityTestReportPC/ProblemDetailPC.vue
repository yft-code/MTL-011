<template>
    <div>
        <el-card v-loading="tableDataLoading" class="box-card" id="problemTotal">
                    ...
                    <el-table 
                        ...
                        <!-- <el-table-column type="expand">
                            <template v-slot="scope">
                                <div style="margin: 10px; background: #eeeeee;">
                                    <record-table :basicColumnData="basicColumnData" :tableData="scope.row.recordList"></record-table>
                                </div>
                            </template>
                        </el-table-column> -->
                        ...
                        <el-table-column
                            :label="item.titleName"
                            v-else-if="item.propName == 'failReason'"
                            align="left"
                            minWidth="120">
                            <template v-slot="scope">
                                <div v-for="item,key in scope.row.inflect_model[0]" :key="key">
                                    <span>{{displayName[key]}} : {{item}}</span>
                                </div>
                            </template>
                        </el-table-column>
                        ...
                        <el-table-column 
                            ...
                            v-else-if="item.propName == 'q_classification' || item.propName == 'q_description'">
                            <template v-slot="scope">
                                <div>{{scope.row[item.propName]}}</div>
                            </template>
                        </el-table-column>
...
import QuestionDrawer from '@/components/TestReport/CompatibilityTestReportPC/QuestionDrawer'
...
export default {
    ...
    data() {
        return {
            ...
            // expertProblemColArr: ["q_category", "q_classification", "q_level", "q_description", "recurrent_rate", "allDevice","questionPic", "test_qa", "follow_qa", "remarks"],
            expertProblemColArr: ["q_category", "q_classification", "q_level", "q_description", "recurrent_rate", "failReason", "questionPic", "test_qa", "follow_qa", "remarks", "questionDetail"],
        }
    },
    props: {
        axiosList: {
    ...
    methods: {
        ...
        getTableData() {
            ...
            this.$codePost('/expertCompatibilityPC/' + this.problemTableList.postApp + '/', list, { operation: "获取数据", failed: true }).then(res => {
                if (res.code == 200) {
                    //表格数据
        ...
        viewDetail(item){
            ...
            //     path: "/mtl_test_platform/page/questionDetailPC",
            //     query: queryList
            // })
        ...
        confirmEditRemarks(){
            ...
            // console.log(list)
            this.$codePost('/expertCompatibilityPC/update_question_remarks/', list, { operation: "修改问题备注", failed: true }).then(res => {
                if (res.code == 200) {
                    this.getTableData()
        ...
        confirmChangeFollowQa(){
            ...
            this.$codePost('/expertCompatibilityPC/update_follow_qa/', list, { operation: "修改问题备注", failed: true }).then(res => {
                if (res.code == 200) {
                    this.getTableData()
