<template>
    <div id="weaknetPCTestReport" class="performance-test-report">
        <!-- 报告标题 -->
        <div class="reportTitle" :style="this.$route.path=='/mtl_test_platform/page/weaknetPCTestReport' ? {'margin-top':'67px'} : {}">
            <span>{{ taskDetail['test_report_title'] || "" }}</span>
        </div>
        <div class="container">
            <!-- 测试信息 -->
            <div>
                <!-- <task-detail v-loading="isLoading" :appUrl="appUrl" :axios-list="axiosList" :isPreview="isPreview" :taskDetail="taskDetail" @afterEdit="afterEditMethod"/> -->
                <task-detail 
                    ...
                    :editUrl="'/pcWeaknetTest/edit_report_msg/'"
            ...
            <!-- 备注 -->
            <remark :isPreview="isPreview" :taskDetail="taskDetail" :updateUrl="'/pcWeaknetTest/edit_report_remarks/'" @cancelEditRemark="cancelEditRemark" @updateRemark="getPageData"></remark>

            <!-- 报告预览页面 -->
            <div v-if="isPreview">
                <test-conclusion :conclusionData="conclusionData"></test-conclusion>
            ...
            <div v-else>
                <div class="selectPackage">
                    <el-radio-group v-model="packageType" @change="changePackageType">
                        <el-radio-button :label="0">测试结论</el-radio-button>
                        <el-radio-button :label="1">测试数据</el-radio-button>
                    </el-radio-group>
                </div> 
                <!--测试结论-->
...
import CurrentTest from '@/components/TestReport/WeaknetPCTestReport/CurrentTest'
import TestConclusion from '@/components/TestReport/WeaknetPCTestReport/TestConclusion'
        ...
        getPageData(){
            ...
            this.$codePost("/" + this.appUrl + "/test_basic_info_report/", this.axiosList, { operation: "获取报告数据", failed: true }).then(res => {
                if (res.code == 200) {
                    ...
                    this.conclusionData = res.data.task_detail.conclusion[0]
